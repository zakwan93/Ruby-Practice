...
def func_all(hash)
    ...
    hash.all? {|key,value| (value < 10) && (value.is_a? Integer) }
end
...
def func_find(hash)
    ...
    new_hash = (hash.find { |key,value| (key.is_a? Integer) && (value.is_a? Integer) && (value < 20)  }) || (hash.find { |key,value| (key.is_a? String) && (value.is_a? String) && (value.start_with? ("a")) })
end
